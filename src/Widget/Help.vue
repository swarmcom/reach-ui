...
Select from the drop down which release group will be available to agent(s). A release group is a collection of release codes that the administrator has defined under the Config - Releases section in the Admin menu of the user portal. The release group selected here will directly determine which release codes this agent(s) will be able to choose from when marking themselves as being in the release state. If no release group is selected on an agent level, the release group will be determined by the release group setting on the agent group that the agent is assigned to.
  </helper>
...
Enter a number of seconds or leave blank. Note that this parameter can be set at both the agent and agent group level with the agent level taking precedence.  If blank at both the agent and agent group level, the agent will never be automatically logged off from the system. If set either here or on the agent group level, and an agent sits in the released mode for this specified number of seconds without having any activity, the system will automatically log them off. If the agent is a persistent agent, this auto logout feature will be ignored.
  </helper>
...
Select a skill group.  A skill group is a set of skills as defined in the Config - Skill groups configuration page.  If a skill group is defined, all skills from the skill group are added to any other individually selected skills to create a complete skill list.  Also note that skills can be assigned at both the agent and agent group levels.  All skills assigned at both levels are aggregated together to create a complete skill list for an agent.
  </helper>
...
Enter an integer value. The weight of the queue represents the priority that will be placed on the calls in the queue. This directly affects how calls will be delivered to agents. The higher the weight the more priority will be placed on the calls in the queue. Note that ALL calls with a weight of 2 will be answered before ANY calls with a weight of 1. So, in an environment where there are always calls in the queue with a weight of 2, it's possible that calls of a lower weight will never be answered. If a less aggressive approach to favoring calls is desired, use the Aging parameter.  Note that weight can be set at the queue or queue group level with the queue level setting taking precedence.
  ...
  <helper target="Aging">
Enter a value to be used to artificial age calls in this queue. The aging factor can include a single decimal place such as 1.2 or 5.3 or can be a whole number such as 1. The aging factor is used to “age” calls at different rates so that the administrator can prioritize calls in one queue over calls in another queue. It differs from the Weight parameter in that calls with a higher aging factor are not ALWAYS answered prior to ANY call that has a lower aging factor. Instead, the actual waiting time of calls in queue are multiplied by the aging factor. The resulting mathmatical products are used to determine which call should be serviced first. Example: call1 has been in queue for 30 seconds and has an aging factor of 1. Call 2 has been in queue for 16 seconds and has an aging factor or 2. Call1 has an effective wait time of 30 and call2 has an effective wait time of 32. So in this case, call2 would be answered before call1 even though call1 has been in queue longer.  Note that aging factor can be assigned at the queue or queue group level with the queue level setting taking precedence if set in both places.
  </helper>

  <helper target="Wrap-up enabled">
Set to True, False or Not Set. This value determines whether or not the wrap up feature is on for calls in this queue. The wrap-up feature is used to provide an agent with “after call” work time. If on, once an agent finishes a call, they are put into a wrap-up state rather than being placed into the available state immediately. This allows the agent to finalize some work on the call that just ended before they get more calls offered to them. If enabled (True), the Wrap-up timer and Auto wrap-up parameters determine how much time an agent is given in the wrap up mode.  Note that Wrap-up enabled can be set at the queue or queue group level with the queue level taking precedence.
  </helper>
...
Enter a number of seconds. This value will be the amount of seconds after an agent entered the wrap up mode that they will automatically be brought back out of wrap mode and placed into available. If this value is not entered, the agent will never be put back to available automatically. This value will only be used if the Wrap-Up enabled parameter is True. Also note that this value can be set on the queue or queue group level with the queue level taking precedence. If it is not set on queue or queue group, it will be treated as blank and the auto wrap-up feature will not apply, meaning that an agent will never be automatically placed back into the available state.
  </helper>

  <helper target="Queue Skills">
Select the required skills from the drop down selector for calls entering this queue. Remove skills by clicking on the skill name to the right of the drop down selector.  Note that skills may also be assigned at the queue group level, line level and client level. Skills assigned at the queue group level are inherited by all queues within the queue group. To reiterate, a calls required skills consist of all the skills assigned to it at the line, client, queue group and queue level. This required skill list is then matched to agents and any agent with AT LEAST the required skills of a call can be offered the call. Also note that skills can be manipulated in recipe steps. This means that not all calls in a queue will necessarily have the same required skills list over time. 
  </helper>
...
Enter a number of seconds. This value will be the amount of seconds after an agent entered the wrap up mode that they will automatically be brought back out of wrap mode and placed into available. If this value is not entered, the agent will never be put back to available automatically. This value will only be used if the Wrap-Up enabled check box is checked. Also note that this value can be set on the queue level. If it is set there, that value will be used. If not, the value will be pulled from here. If it is not set on queue or queue group, it will be treated as blank and the auto wrap-up feature will not apply.
  ...
  <helper target="Skills">
Select the required skills from the drop down selector for calls entering this queue. Remove skills by clicking on the skill name to the right of the drop down selector.  A calls required skills consist of the entire list of skills assigned at all of these levels.  Skills assigned at the queue group level are inherited by all queues within the queue group. To reiterate, line, client, queue group and queue level skills assignments are aggregated together to create a full list of required skills.  This required skill list is then matched to agents and any agent with AT LEAST the required skills of a call can be offered the call. Also note that skills can be manipulated in recipe steps. This means that not all calls in a queue will necessarily have the same required skills list over time. 
  </helper>
