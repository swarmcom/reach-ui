...
<div v-access:profile-ui class="form" v-bind:class="classObject">
  <form-text :disable="disableComponent('agentName-edit')" label="Agent Name" v-model="rec.name"></form-text>
  <form-text label="Login" v-model="rec.login"></form-text>
  <form-text :disable="disableComponent('agentPassword-edit')" label="Password" v-model="rec.password"></form-text>
  ...
  <lines v-if="!disableComponent('agentLineOut-visible')" label="Line Out" v-model="rec.line_id"></lines>
  <roles label="Role" v-model="rec.role_id" :effective="eff.role_id"></roles>
  <form-text :disable="disableComponent('primarySip-edit')" label="Primary Contact" v-model="rec.uri"></form-text>
  <uris v-if="!disableComponent('additionalSip-visible')" label="Additional Contacts" v-model="rec.uris"></uris>
  <form-text label="Agent Caller ID Number" v-model="rec.caller_id_number"></form-text>
  <form-text :disable="disableComponent('ringTimeout-edit')" label="Ring Timeout" v-model="rec.ring_timeout" :effective="eff.ring_timeout"></form-text>
  <form-text label="Suspend Time" v-model="rec.suspend_time"></form-text>
  <form-text :disable="disableComponent('maxMissedCalls-edit')" label="Max Missed Calls (auto release)" v-model="rec.max_ring_fails" :effective="eff.max_ring_fails"></form-text>
  <form-select-bool :disable="disableComponent('maxRingsSuccess-edit')" label="Reset Max Rings On Success" v-model="rec.reset_rings_fails" :effective="eff.reset_rings_fails"></form-select-bool>
  <form-text :disable="disableComponent('autoLogout-edit')" label="Auto Logout" v-model="rec.autologout" :effective="eff.autologout"></form-text>
  <form-file v-if="!disableComponent('avatar-visible')" label="Avatar" uri="/avatar" v-model="rec.avatar"></form-file>
  <form-tags v-if="!disableComponent('agentSkills-visible')" label="Agent Skills" placeholder="Skill..." v-model="skills" :effective="effective_skills"></form-tags>
